...
사용법:
    ...
    # 60초마다 반복 (Ctrl+C 종료). 기본은 '멈춤'으로 시작 → 대시보드(/collector)에서 '재개'해야 수집.
    python3 -m engine.collector BTCUSDT --loop 60
    # 시작하자마자 수집하려면(대시보드 없이):
    python3 -m engine.collector BTCUSDT --loop 60 --start-running
...
def main():
    ...
    ap.add_argument("--start-running", action="store_true",
                    help="시작 시 바로 수집 활성. 기본은 '멈춤'으로 시작(대시보드에서 재개). "
                         "배포(compose)는 이 플래그로 24/7 수집.")
    args = ap.parse_args()
    ...
    from . import control
    # 안전 기본값: 트레이더와 동일하게 '멈춤'으로 시작 → 대시보드에서 명시적으로 '재개'해야 수집 시작.
    # (로컬 개발 중 캔들 수집이 저절로 도는 걸 막음. 배포 compose는 --start-running 으로 24/7 유지.)
    if not args.start_running:
        control.set_service("collector", "paused")
        print("🔒 안전 시작: 수집 '멈춤' 상태 — 대시보드에서 '재개'해야 캔들 수집이 시작됩니다.", flush=True)
