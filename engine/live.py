...
def notify(msg: str) -> None:
    """선택적 알림 — 봇 토큰이 있으면 봇으로, 없으면 웹훅으로 POST. 없으면 무시. stdlib만.

    우선순위: DISCORD_BOT_TOKEN + DISCORD_CHANNEL_ID(봇 발신) > NOTIFY_WEBHOOK(웹훅).
    봇으로 보내면 발신자가 내 봇으로 통일되고, 이후 메시지에 버튼(components)을 붙일 수 있다
    (웹훅 메시지엔 인터랙티브 버튼을 못 단다 → 그래서 봇으로 옮긴다). 봇 전송도 상주 Gateway가
    아니라 그냥 REST POST 한 방이라 여기(trader)에 discord.py 는 필요 없다.
    웹훅은 Slack 도 지원(payload 키가 다름: Slack=text, Discord=content)하므로 폴백으로 남긴다.
    """
