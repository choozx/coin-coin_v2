...
def api_keys(testnet: bool):
    """네트워크별 API 키 (key, secret). 테스트넷과 메인넷은 **다른 계정**이라 키도 다르다.

    네트워크 전용 변수를 먼저 보고, 없으면 공용 BINANCE_API_KEY/SECRET 로 폴백한다
    (키 쌍 하나만 쓰던 기존 .env 호환). 폴백한 키가 그 네트워크 것이 아니면 인증이 실패할 뿐
    — 엉뚱한 계정에 주문이 나가는 일은 없다(키는 네트워크 전용이라 교차 인증이 안 된다).
    """
    prefix = "BINANCE_TESTNET" if testnet else "BINANCE_MAINNET"
    key = os.environ.get(f"{prefix}_API_KEY") or os.environ.get("BINANCE_API_KEY")
    secret = os.environ.get(f"{prefix}_API_SECRET") or os.environ.get("BINANCE_API_SECRET")
    return key, secret
...
class LiveExecutor(Executor):
    ...
    **실제 시장은 그 가격에 안 준다.** 이 클래스의 일은 주문을 내고 돌아온 진짜 체결가·수량·
    ...
    def __init__(self, testnet: bool = None, symbol: str = None,
        ...
        self.allow_mainnet = bool(allow_mainnet) or not self.testnet
        self.api_key, self.api_secret = api_keys(self.testnet)
        self.symbol = symbol
    ...
    def _require_keys(self) -> None:
        if self.api_key and self.api_secret:
            return
        net = "테스트넷" if self.testnet else "메인넷"
        var = "BINANCE_TESTNET_API_KEY" if self.testnet else "BINANCE_MAINNET_API_KEY"
        raise RuntimeError(
            f"실거래({net}): API 키가 없습니다(.env) — {var}/_SECRET 또는 공용 BINANCE_API_KEY/_SECRET. "
            "출금권한 OFF·IP 화이트리스트 필수.")
    ...
    def set_network(self, network: str) -> None:
        """거래소 네트워크 교체(테스트넷 ↔ 메인넷). 무포지션일 때만.

        플래그 하나 뒤집는 게 아니라 **다른 계정으로 갈아타는 것**이다 — 키·잔고·포지션·원장이
        전부 바뀐다. 그래서 ① 실돈 권한(--real-money)이 있는지 ② 그 네트워크 키가 있는지 ③ 계정이
        멀쩡한지(preflight) 를 다 확인하고, 하나라도 실패하면 **원래 네트워크로 되돌린다** —
        절반만 바뀐 상태로 매매가 이어지는 게 제일 나쁘다.
        """
        if network not in ("testnet", "mainnet"):
            raise ValueError("network는 testnet 또는 mainnet")
        if network == self.network:
            return
        if self.position is not None:
            raise RuntimeError(f"포지션 보유 중엔 네트워크 변경 불가: {self.network} → {network}")
        if network == "mainnet" and not self.allow_mainnet:
            raise RuntimeError(
                "실돈(메인넷) 권한이 없는 프로세스입니다 — 봇을 --real-money 로 띄워야 "
                "대시보드에서 실돈으로 전환할 수 있습니다.")
        before = (self.testnet, self.api_key, self.api_secret, self._broker)
        ...
        try:
            ...
            if self._own_broker:
                from .binance_broker import BinanceBroker
                self._broker = BinanceBroker(self.api_key, self.api_secret, self.testnet, self.symbol)
            self.preflight()                 # 새 계정이 원웨이·격리·조회 가능한지 확인하고 나서 확정
        except Exception:
            self.testnet, self.api_key, self.api_secret, self._broker = before
            self._equity_cache = (0.0, 0.0)
            raise
