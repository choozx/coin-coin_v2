...
| **전략 엣지 (실돈을 걸 근거)** | ❌ **방향성 7연속 기각.** 단 M(펀딩 횡단면)이 조건부 후보 — **유동성 상위 30·k=7 로 자본 10만~50만$** (용량 한계) ([`research/BACKLOG.md`](research/BACKLOG.md)). 실돈 보류 |
| 블록 빌더 UI | ⬜ 예정 |

> **읽는 순서 주의.** 위 표의 ✅ 는 **배선이 동작한다**는 뜻이지 **돈을 번다**는 뜻이 아니다.
> 엣지 연구는 일곱 번 기각됐고(K 는 실제로 돌리던 라이브 프리셋이었다) **실돈 전환은 보류**다.
> M(펀딩 횡단면)만 조건부 후보로 살아 있는데, 용량이 10만~50만$ 이고 다심볼 실행 엔진이 없다. 테스트넷 배선 검증은 계속 굴린다 — 어떤 전략을 쓰든 필요한
...
entry_log.py    진입 판정 로그 (data/entry_log.jsonl) — 폴링마다 '왜 안 샀나'를 한 줄씩
...
봇이 조용히 아무것도 안 할 때, 그게 **조건이 안 맞아서인지 · 멈춰서인지 · 쿨다운인지 ·
워밍업인지** 밖에서는 구별할 수 없었다. 상태 파일은 '무포지션'만 알려주고 이유를 안 남긴다.

이제 트레이더가 폴링마다 판정 한 줄을 `data/entry_log.jsonl` 에 남긴다 — 조건별 참/거짓,
지표 현재값, 막힌 사유. 같은 내용이 대시보드(무포지션일 때 표시)와 `docker compose logs
trader` 에도 나온다.

```
[진입판정] 진입✗ [판정] 진입 조건 미충족 · long ✗ SUPERTREND_DIR(10,3.0) -1→-1 상향돌파 0 / RVOL(20)=0.76534 > 1.5 외 1개
  (long)
    ✗ 모두 (0/3 충족)
      ✗ SUPERTREND_DIR(10,3.0) -1→-1 상향돌파 0
      ✗ RVOL(20)=0.76534 > 1.5
      ✗ TAKER_DELTA_RATIO=0.25466 > 0.3
```

**어느 조건이 막고 있나**가 값과 함께 나온다. 조건이 붙는 과정도 봉 단위로 보인다:

```
08:03  [미리보기] ✗ SUPERTREND_DIR(10,3.0) -1→-1 상향돌파 0 · ✗ RVOL(20)=1.399 > 1.5
08:04  [미리보기] ✗ SUPERTREND_DIR(10,3.0) -1→-1 상향돌파 0 · ✓ RVOL(20)=1.74 > 1.5
08:05  [  판정  ] ✓ SUPERTREND_DIR(10,3.0) -1→1 상향돌파 0 · ✓ RVOL(20)=2.131 > 1.5 → 🟢 진입 롱
```

각 규칙(롱/숏 그룹)은 `lines`(트리 전체)와 `failed`(거짓인 말단 조건만) 둘을 남긴다.
`failed` 는 "무엇이 막고 있나"의 답이라 요약 한 줄과 `jq` 질의가 트리를 다시 파싱하지 않아도 된다:

```bash
jq -r 'select(.block=="진입 조건 미충족") | .rules[] | "\(.side): \(.failed|join(" / "))"' \
   data/entry_log.jsonl | sort | uniq -c | sort -rn      # 무엇이 가장 자주 막았나
```

`decided` 로 두 종류를 구분한다: **true** = 신호봉이 닫혀 진짜 판정이 일어난 순간(15m
프리셋이면 15분마다), **false** = 그 사이 폴링의 미리보기(아직 안 닫힌 봉의 지금 값).
미리보기가 없으면 15분에 한 줄이라 그동안 지표가 어디쯤인지 볼 수 없고, 그렇다고 미완성
봉의 값을 확정처럼 적으면 거짓말이 된다 — 그래서 남기되 구분한다.

차단 사유는 판정 순서 그대로다: 포지션 보유 → 지정가 대기 → 진입 게이트(멈춤·가드레일) →
필터(쿨다운·펀딩 임박·거래시간) → 조건 미충족. **판정과 로그는 같은 코드**(`Stepper.entry_block`)
를 쓴다 — 갈라지면 로그가 거짓말을 하고, 그러면 없느니만 못하다.

기록은 관찰용이라 어떤 실패도 매매를 막지 않는다(예외를 삼킨다). 파일은 `ENTRY_LOG_KEEP`
줄(기본 3000 ≈ 폴링 1분으로 이틀)로 잘라 유지하고, `0` 이면 기록을 끈다. 재시작 직후
캔들을 따라잡으며 재평가하는 과거 봉은 남기지 않는다(기동 때마다 수백 줄이 쏟아지므로).
