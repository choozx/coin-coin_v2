...
| **전략 엣지 (실돈을 걸 근거)** | ❌ **방향성 7연속 기각.** 단 M(펀딩 횡단면)이 조건부 후보 — **자본 1만~10만$ 에서만** 성립(용량 한계) ([`research/BACKLOG.md`](research/BACKLOG.md)). 실돈 보류 |
| 블록 빌더 UI | ⬜ 예정 |

> **읽는 순서 주의.** 위 표의 ✅ 는 **배선이 동작한다**는 뜻이지 **돈을 번다**는 뜻이 아니다.
> 엣지 연구는 일곱 번 기각됐고(K 는 실제로 돌리던 라이브 프리셋이었다) **실돈 전환은 보류**다.
> M(펀딩 횡단면)만 조건부 후보로 살아 있는데, 용량이 1만~10만$ 뿐이고 다심볼 실행 엔진이 없다. 테스트넷 배선 검증은 계속 굴린다 — 어떤 전략을 쓰든 필요한
