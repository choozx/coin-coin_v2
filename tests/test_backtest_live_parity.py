...
왜 필요한가: 예전엔 두 경로가 per-bar 로직을 **각각 구현**했다 — backtest.run() 의 루프와
LiveTrader._step() 이 "동일 순서"라는 주석으로만 맞춰져 있었다. 사이징·청산가 같은 primitive
는 공유해도 **순서와 조건은 사람이 맞춰둔 것**이라, 한쪽만 고쳐도 나머지 테스트는 전부
통과했다. 그 차이는 실거래에서야 드러난다(이 테스트가 실제로 세 건을 잡아냈다).

지금은 판정이 backtest.Stepper.step() 한 곳뿐이고 두 경로가 그걸 함께 부른다. 그래서 이
테스트의 역할이 바뀌었다 — 일치를 *발견*하는 게 아니라, 누군가 라이브 쪽에 판정을 다시
심어 두 경로가 **또 갈라지는 것**을 막는 울타리다. 공유 구조를 되돌리면 여기서 깨진다.
