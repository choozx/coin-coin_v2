...
- [ ] **8-1. 테스트넷(가짜돈)으로 며칠** — EC2 `.env` 에 `BINANCE_TESTNET=1` + 테스트넷 키.
  ...
  - 백테스트 가정 vs 실제: **슬리피지 · maker 체결 비율 · 펀딩** →
    원장엔 안 남는다(체결가가 덮어써지고 maker/taker 컬럼이 없다). `data/fill_log.jsonl`
    (`engine/fill_log.py`) 에 쌓이며 **`sudo python3 tools/report.py`** 로 본다.
    ⚠ 테스트넷은 호가가 얇아 **슬리피지 절대값은 못 믿는다** — 여기서 검증되는 건 배선
    (주문 경로·부분체결·거부·재시작 인계·펀딩 정산)이고, 수치 교정은 메인넷 소액에서 한다.
    ⚠ 원장은 WAL 이라 읽기에도 -shm 쓰기가 필요하다 → 컨테이너가 만든 파일이면 `sudo` 로 볼 것.
  - 컨테이너를 일부러 재시작해 **포지션 인계**가 되는지 (`data/live_position.json`)
- [ ] ⛔ **8-2. 실돈 전환 — 지금은 보류다.** 배선이 아니라 **근거**가 없어서다:
      엣지 연구가 여섯 번 기각으로 종결됐고(`research/BACKLOG.md` 의 '현재 상태'), 마지막 K 는
      **바로 이 단계에서 돌리던 라이브 프리셋**이 귀무 p95 를 못 넘은 것이다.
      아래 절차는 재개 조건을 충족한 뒤에 밟는다. **새 프리셋은 실거래 전 귀무모델 판정부터.**

      8-1 처럼 `--real-money` 로 띄웠다면 **대시보드 컨트롤바의 거래소
...
- [x] 테스트 게이트 — 테스트가 통과해야 이미지가 나간다 (`.github/workflows/deploy.yml`, e085082)
- [ ] ⏸ 자동배포 승인 규칙: 나쁜 push가 실매매 봇 갈아치우지 않게 GitHub Environment 승인 규칙
      (실돈 전환과 함께 보류 — 걸 만한 전략이 생기면 그때)
